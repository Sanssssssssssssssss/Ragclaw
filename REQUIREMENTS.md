...
### FR-007 VS Code 本地演示与学习配置
- 状态：Confirmed
- 项目需要补齐适合本地 demo 和学习的 VS Code 配置。
- 验收标准：
  - 仓库内存在可复用的 VS Code 配置文件。
  - 至少包含 Python 解释器设置、前后端启动任务、推荐扩展或调试入口。

### FR-008 一键启动入口
- 状态：Confirmed
- 项目需要提供从仓库根目录同时启动前后端的脚本，降低 demo 与学习成本。
- 验收标准：
  - 仓库根目录存在一键启动脚本。
  - 脚本可启动前端与后端，或在依赖缺失时给出明确提示。

### FR-009 Kimi 模型配置入口
- 状态：Confirmed
- 项目需要支持将默认聊天模型切换到 Kimi，并明确 API Key 的填写位置。
- 验收标准：
  - 后端配置层支持 Kimi。
  - 文档明确说明在何处填写 Key、Base URL 和模型名。
...
### FR-010 工具模型与回答模型分离
- 状态：Confirmed
- 当主聊天模型使用 Kimi K2.5 时，系统需要支持为工具调用型 agent 单独配置更稳定的工具模型，而不影响主回答模型。
- 验收标准：
  - 主回答链路仍可保持 `kimi-k2.5`
  - 工具调用型 agent 可通过独立配置成功完成至少一次真实工具调用

### FR-011 可验证的向量检索链路
- 状态：Confirmed
- 系统需要提供可运行的向量检索路径，优先允许在没有额外商业 embedding key 时使用本地 embedding provider 完成学习和 demo。
- 验收标准：
  - 知识索引状态中 `vector_ready=true`
  - 仓库内存在独立的向量检索验证脚本
  - 至少一次真实知识检索链路可同时出现向量或混合检索结果
